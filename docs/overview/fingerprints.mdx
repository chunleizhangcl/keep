...
1. **Provider/custom rule calculation**: Each provider can declare `FINGERPRINT_FIELDS`. During alert formatting, `BaseProvider.get_alert_fingerprint()` iterates those fields in order, walks nested keys (e.g., `labels.instance`), JSON-serializes list/dict values, and feeds each value (as UTF-8 bytes) into a SHA-256 hash. The resulting hex digest is used as the fingerprint. If the list is empty, it returns `alert.name`.
